...
class GraphBoundaryCycle:
    ...
    def generate_geometry_from_graph(self):
        ...
        for ix, c in enumerate(nx.simple_cycles(G=self.G, length_bound=3)):
            self.inner_faces[ix] = {"cycle": c, "shape": sp.LinearRing(self.get_emedding_coords(self.embed, c))}

    def get_geometry_boundary(self):
        complete_faces = sp.MultiPolygon([sp.Polygon(i["shape"]) for i in self.inner_faces.values()])
